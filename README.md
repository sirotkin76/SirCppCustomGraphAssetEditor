...
11. Обе ноды имеют атрибут **UniqueTagToSearchForThisNode** типа тег. Базовое значение в том, что бы задавать уникальный тег для данной ноды и используя функцию **SirCppFindNodeUse_TagForFindNode** находить уникальный индекс ноды с текущим уникальным тегом. (Например вы делаете новелу и вам нужно возвращаться на кнопку back назад, на предыдущую ноду или искать в графе какую то уникальную ноду, которая может быть вообще не подключена. )
    
![image](Pic/Function1.png)
