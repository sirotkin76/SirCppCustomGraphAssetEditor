...
9. Функция **SirCppFindNode_END_GetAttrData_01**
   Получаем из базовой функции уникальное значение ноды и проверяем тип ноды, относится ли она к END. Если да, то в переменной графа находим функцию **SirCppFindNode_END_GetAttrData_01** передаем туда уникальное значение. На прямую или через переменную. После чего на выходе можем работать уже со значениями из данной ноды.
